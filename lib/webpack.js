...
module.exports = function setupWebpack(config, server, onCompilation) {
  ...
  config.webpack.plugins.push(new webpack.ProgressPlugin((pct, message, addInfo) => {
    // console.log(pct, message, addInfo)
    if (pct > 0 && pct < 1)
      onCompilation({status: 'compiling', percentage: Math.round(pct * 100), message})
  }))
