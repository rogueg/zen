...
module.exports = class WebpackAdapter extends EventEmitter {
  constructor () {
    ...
    this.compiler.hooks.compilation.tap('Zen', compilation => {
      compilation.hooks.processAssets.tap('Zen', assets => {
        files = Object.entries(assets).map(([pathname, source]) =>
          ({ path: `webpack/${pathname}`, body: source.source() })
        )
      })
    })
    this.compiler.hooks.done.tap('Zen', (stats, callback) => {
      ...
      this.onStats({ hash: stats.hash, files, percentage: 100, status: 'done' })
  ...
  async build () {
    return await new Promise((resolve, reject) => {
      this.compiler.run(function (error, stats) {
        ...
        if (stats.errors && stats.errors.length > 0) return reject(new Error(stats.errors[0]))
        resolve(stats)
      })
    })
  }
