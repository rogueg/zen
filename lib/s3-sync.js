...
module.exports = class S3Sync {
  ...
  async applyPatch(file) {
    ...
    let patches = dmp.patch_make(lastWrite, file.body.toString())
    let result = await util.invoke(this.lambda, 'serverless-zen-dev-applyPatch', {
      bucket: this.config.aws.assetBucket,
      key: file.key,
      md5: crypto.createHash('md5').update(lastWrite).digest('hex'),
  ...
  async upload(file) {
    let result = await this.s3.upload({
      ...
      Key: file.key,
      Body: file.body,
      ContentType: file.contentType || mime.contentType(file.path)
    }).promise()
  }
}
