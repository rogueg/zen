...
module.exports = class S3Sync extends EventEmitter {
  ...
  async run(index) {
    ...
    await workInPool(Object.values(this.files), 10, async f => {
      ...
      if (snap && snap.mtime && snap.mtime === f.mtime)
        f.hash = snap.hash // the mtime is unchanged, so our hash is still valid

      if (!snap || !snap.hash || snap.hash !== f.hash)
    ...
    let manifest = {
      ...
      proxyUrl: Zen.config.proxyUrl,
      s3Url: Zen.config.s3Url,
      files: Object.values(this.files).map(f => Object.select(f, ['urlPath', 'toCheck', 'versionedPath'])),
      index
    }
    let {needed} = await Util.invoke('serverless-zen-dev-sync', manifest)
    ...
    await workInPool(needed, 20, async need => {
      ...
      let result = await this.s3.upload({Bucket: Zen.config.aws.assetBucket, Key, Body, ContentType}).promise()
