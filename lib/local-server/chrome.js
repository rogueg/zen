...
module.exports = class ChromeWrapper {
  ...
  // https://github.com/GoogleChrome/puppeteer - useful reference for Google's canonical approach
  // https://github.com/adieuadieu/serverless-chrome/tree/master/packages/lambda - the old launcher I used to use
  // https://github.com/GoogleChrome/chrome-launcher/ - yet another launcher
  async launchLambda() {
    let flags = [
      '--disable-background-timer-throttling', '--disable-breakpad', '--disable-extensions',
      '--disable-client-side-phishing-detection', '--disable-cloud-import', '--disable-default-apps', '--disable-popup-blocking',
      '--disable-dev-shm-usage', '--disable-gesture-typing', '--disable-print-preview', '--disable-prompt-on-repost',
      '--disable-hang-monitor', '--disable-infobars', '--disable-notifications', '--disable-offer-store-unmasked-wallet-cards',
      '--disable-offer-upload-credit-cards', '--disable-setuid-sandbox', '--disable-speech-api',
      '--disable-sync', '--disable-tab-for-desktop-share', '--disable-translate', '--disable-voice-input', '--disable-wake-on-wifi',
      '--enable-async-dns', '--enable-simple-cache-backend', '--enable-tcp-fast-open', '--hide-scrollbars', '--media-cache-size=33554432',
      '--metrics-recording-only', '--mute-audio', '--no-default-browser-check', '--no-first-run', '--no-pings', '--no-sandbox',
      '--no-zygote', '--password-store=basic', '--prerender-from-omnibox=disabled', '--use-mock-keychain', '--memory-pressure-off',
      '--enable-webgl', '--ignore-gpu-blacklist', '--use-gl=swiftshader',
      '--headless', '--single-process', '--remote-debugging-port=9222', '--user-data-dir=/tmp/chromeUserData'
    ]
    ...
    process.env.FONTCONFIG_PATH = '/opt/lib' // not sure why, but this seems to work better than passing it to spawn

    this.process = spawn('/opt/bin/chromium', flags, {
      detached: true,
      env: Object.assign({TZ: 'America/New_York'}, process.env),
      stdio: ['ignore', fs.openSync('/tmp/chrome-out.log', 'a'), fs.openSync('/tmp/chrome-err.log', 'a')]
    })
    console.log('Chrome spawned')

    // Repeatedly try and open a socket to the devtools port
    let connected = false, socket, attempts = 0
    while (!connected) {
    ...
    if (!connected) {
      this.dumpChromeLogs()
      throw new Error('timed out trying to connect to devtools')
    }
  ...
  async openTab(url, id, config = {}, manifest) {
    try {
      ...
      let ua = await cdp.Runtime.evaluate({expression: 'navigator.userAgent', returnByValue: true})
  ...
  dumpChromeLogs () {
    console.log(fs.readFileSync('/tmp/chrome-out.log').toString())
    console.log(fs.readFileSync('/tmp/chrome-err.log').toString())
  }
}
...
class ChromeTab {
  ...
  getTestNames() {
    let promise = new Promise((resolve, reject) => this.listRequest = {resolve, reject})
    return promise
  }
  ...
  async listTests() {
    let {result, exceptionDetails} = await this.cdp.Runtime.evaluate({expression: `Latte.flatten().map(t => t.fullName)`, returnByValue: true})
    if (exceptionDetails) {
  ...
  async onRequestPaused({requestId, request}) {
    ...
    if (path.match(/^index\.html/)) {
      ...
      return this.cdp.Fetch.fulfillRequest({requestId, responseCode: 200, responseHeaders, body})
    }
    ...
    if (key) {
      ...
      let body = Buffer.from('Missing from manifest', 'binary').toString('base64')
      this.cdp.Fetch.fulfillRequest({requestId, responseCode: 404, responseHeaders, body})
    }
  }
}
