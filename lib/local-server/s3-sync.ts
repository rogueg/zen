...
export default class S3Sync extends EventEmitter {
  ...
  constructor() {
    ...
    if (Zen.config.aws.s3CacheVersion && this.snapshot.version !== Zen.config.aws.s3CacheVersion) {
      this.snapshot = {}
    }
  }
  ...
  async run(index) {
    ...
    let syncedDirs = [
      {filePath: path.join(__dirname, '../client'), webPath: '/lib/client', filter: (p) => p.match(/worker.js|latte.js/)}
