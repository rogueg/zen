...
async function runOnLambda() {
  ...
  runGroups.forEach(async (group, index) => {
    let url = config.s3Url + `?id=L${index}` // give each lambda worker an id so the sessionId is useful
    ...
    if (response.errorMessage) {
      ...
      onResults(group.tests.map(fullName => {
        return {runId, error: 'Timeout running on lambda', fullName}
      }))
      return
    }
