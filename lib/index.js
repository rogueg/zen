...
app.use((req, resp) => {
  ...
  let deps = ['/lib/latte.js', '/svelte/mini.js', `/lib/${pageType}.js`]
  deps = deps.concat(testDependencies.map(t => t.replace(appRoot, '/base')))
  if (config.webpack)
    deps.push('//localhost:3101/bundle.js')
...
function run(msg) {
  ...
  else if (grep || msg.grep) { // user specifies what to run
...
async function runOnLambda() {
  await s3Sync.run()
  let tranches = workingSet.inGroupsOf(3)
  console.log(`Using ${tranches.length} workers`)

  tranches.forEach(async toRun => {
    let Payload = JSON.stringify({url: config.s3Url, toRun, runId})
    let response = await lambda.invoke({FunctionName: 'serverless-zen-dev-workTests', Payload}).promise()
...
if (config.webpack) {
  ...
  compiler.plugin("done", (stats) => {
    ...
    lastCompile = stats
    s3Sync.compilationChanged(stats)
    run({codeChange: true})
  })
