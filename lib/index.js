...
config.tmpDir = config.tmpDir || path.join(os.tmpdir(), 'zen')
...
function run(msg) {
  ...
  } else if (msg.codeChange) {
    if (!grep) return sendStatus()
...
async function runWorker(w) {
  while (remaining.length > 0) {
    let result = await w.tab.setTest({runId, testName: remaining.pop()})
