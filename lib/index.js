...
let chrome = new ChromeWrapper() // headless chrome instance
...
async function runOnLambda() {
  ...
  // runGroups = shuffleSeed.shuffle(workingSet, 'yo').inGroupsOf(5)
  lambdaCount = runGroups.length
  sendStatus()

  runGroups.forEach(async group => {
    let response = await Util.invoke(lambda, 'serverless-zen-dev-workTests', {url: config.s3Url, testNames: group.tests, runId})
...
async function runWorker(w) {
  while (remaining.length > 0) {
    let result = await w.tab.assignTest({codeHash: compile.hash, runId, testName: remaining.pop()})
...
function indexHtml(pageType, forS3) {
  ...
  if (forS3) {
    ...
    deps.push(compile.entrypoints.map(e => `webpack/${e}`))
  } else {
    deps.push(testDependencies.map(t => t.replace(appRoot, '/base')))
    deps.push(compile.entrypoints.map(e => `//localhost:3100/webpack/${e}`))
  }
