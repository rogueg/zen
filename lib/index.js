...
async function runOnLambda() {
  ...
  runGroups.forEach(async (group, groupIndex) => {
    let url = config.proxyUrl + `/index.html?id=L${groupIndex}` // give each lambda worker an id so the sessionId is useful
    let tStart = new Date()
    try {
      let response = await Util.invoke(lambda, 'serverless-zen-dev-workTests', {url, testNames: group.tests, runId, Bucket: config.aws.assetBucket, sessionId: config.sessionId})
      ...
      onResults(group.tests.map((fullName, testNumber) => {
        return {runId, error: e.message, fullName, sessionId: groupIndex, testNumber}
      }))
    }
  })
}
