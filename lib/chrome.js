...
module.exports = class ChromeWrapper {
  ...
  async openTab(url, id, config = {}) {
    ...
    console.log(`Connected to ${ua}`) // Useful when debugging different versions

    let tab = new ChromeTab(cdp, id, config)
...
class ChromeTab {
  ...
  async onRequestPaused({requestId, request}) {
    ...
    if (key) {
      let url = `${this.manifest.s3Url}/${key}`
