...
(function() {
  ...
  window.helper = function(name, fnOrGroup) { current.helpers[name] = fnOrGroup }
  ...
  Latte.clean = async function latteClean() {
    // clean up from a previous test run. Delaying `after` blocks like this means that
    // when a test fails, we leave all the DOM/js in place for you to inspect and debug further.
    if (!currentContext) return

      // TODO run after
      currentContext = null
    // TODO: if after handlers throw (or timeout), we need to refresh the page.
  }

  Latte.run = async function latteRun(selection, {mode}) {
    abort = false
    ...
    let toRun = selection(Latte.root, allTests)
    ...
    for (test of toRun) {
      ...
      if (mode == 'debug') {
        ...
        try { await test.fn.apply(currentContext) }
        catch (e) { console.error(e, e.stack) }
      }
    }

    if (mode != 'debug')
      await changeContext(lastTest, null)
  ...
  async function changeSuite(context, nextSuite) {
    ...
    let chain = currLineage.slice(0, currLineage.indexOf(commonAncestor))
    for (suite of chain) {
    ...
    chain = nextLineage.slice(0, nextLineage.indexOf(commonAncestor)).reverse()
    for (suite of chain) {
