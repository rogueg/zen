...
(function() {
  ...
  Latte.run = async function latteRun(suiteName, testNames, {mode}) {
    ...
    if (currentContext) {
      await applyTopDown(currentContext, '_afterEach')
      await applyTopDown(currentContext, '_after')
      currentContext = null
    ...
    for (test of tests) {
      ...
      // clean up from the previous test. This only applies if tests.length > 1
      if (currentContext)
        await applyTopDown(currentContext, '_afterEach')
        if (abort) return

      currentContext = Object.create(suiteRun)
      await applyTopDown(currentContext, '_beforeEach')
      if (abort) return
      await runTest(test, currentContext, mode)
    }
