...
(function() {
  ...
  Latte.run = async function latteRun({tests, mode, onTest, onTimeout}) {
    ...
    for (test of tests) {
      ...
      if (currentContext && Latte.mode == 'debug')
      ...
      if (Latte.mode == 'headless') {
        ...
        catch (e) { Latte.onTest({test, error: e || 'undefined error'}) }
      }
  ...
  async function runWithTimeout(cbOrTest, context) {
    ...
    return Promise.race([timeoutPromise, cbOrTest.fn.call(context)])
