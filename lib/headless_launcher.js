...
launchChrome().then(launcher => {
  chrome({target: `ws://localhost:${port}/devtools/browser`}).then(browser => {
    const { Target } = browser;
    const builds = [
      "file:///Users/stephendemjanenko/Code/zen/lib/headless_test_tab1.html",
      "file:///Users/stephendemjanenko/Code/zen/lib/headless_test_tab2.html",
    ].map(url => {
    ...
    Promise.all(builds).then((values) => {
