...
module.exports.workTests = async (opts, context) => {
  ...
  let hasTimeRemaining = true, attempts = 1, deflakeLimit = opts.deflakeLimit || 1
  while (attempts < deflakeLimit && hasTimeRemaining && results.find(r => r.error)) {
    attempts++
...
module.exports.sync = async (manifest) => {
  ...
  await Promise.all(toCheck.map(async f => {
    try {
      let resp = await s3.headObject({Bucket: process.env.ASSET_BUCKET, Key: f.versionedPath}).promise()
...
module.exports.routeRequest = async (event) => {
  ...
  return {statusCode: 301, headers: {Location: `${manifest.assetUrl}/${encodeURIComponent(key)}`}}
}
...
async function prepareChrome (opts) {
  ...
  let tab = await wrapper.openTab(process.env.GATEWAY_URL + '/index.html', null, {}, manifest)
  return tab
}
...
async function getManifest (sessionId) {
  try {
    let s3 = new AWS.S3()
    let resp = await s3.getObject({Bucket: process.env.ASSET_BUCKET, Key: `session-${sessionId}.json`}).promise()
