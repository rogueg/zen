...
function filterTests(opts = {}) {
  ...
  let tests = Latte.flatten().filter(
    (t) => !grep || grepRegex.test(t.fullName)
  );
...
function runBatchForFocus({ s3 } = {}) {
  ...
  let jsonUrl = encodeURIComponent(
    JSON.stringify(batch.map((r) => r.fullName))
  );
...
async function runIfCodeChanged() {
  ...
  while (
    window.ZenWebpackClient &&
    ZenWebpackClient.needsUpdate(store.get().compile.hash)
  )
